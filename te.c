...
main(int argc, char** argv)
{
	...
	lp_arr = (char**)alloc512(&lp_arr_bank, cf_mx_lines * 2);

	/* alloc text lines dynamic memory banks array */
	lp_arr_i_bank = alloc512(&lp_arr_i_bank_bank, cf_mx_lines);
	...
	if(lp_arr && lp_arr_i_bank) 
	{
		...
		if((clp_line = alloc512(&clp_line_bank, ln_max + 1))) 
		{ 
#endif
			i = 0;
#if OPT_BLOCK
